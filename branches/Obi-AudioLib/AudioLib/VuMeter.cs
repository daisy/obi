...
namespace AudioLib
{
	...
	{
		public event AudioLib.Events.VuMeter.PeakOverloadHandler PeakOverload;        //
        public AudioLib.Events.VuMeter.LevelTooLowHandler LevelTooLowEvent; // event to notify low amplitude
		public event AudioLib.Events.VuMeter.ResetHandler ResetEvent;                 //
		public event AudioLib.Events.VuMeter.UpdateFormsHandler UpdateForms;          //
        public event AudioLib.Events.VuMeter.UpdatePeakMeterHandler UpdatePeakMeter;  //
        public AudioLib.Events.VuMeter.LevelGoodHandler LevelGoodEvent;
        ...
        {
            mPlayer.UpdateVuMeter += new AudioLib.Events.Player.UpdateVuMeterHandler(CatchUpdateVuMeterEvent);
            mPlayer.ResetVuMeter += new AudioLib.Events.Player.ResetVuMeterHandler(CatchResetEvent );
            mRecorder.UpdateVuMeterFromRecorder += new AudioLib.Events.Recorder.UpdateVuMeterHandler(CatchUpdateVuMeterEvent);
            mRecorder.ResetVuMeter += new AudioLib.Events.Recorder.ResetVuMeterHandler(CatchResetEvent);
        }
		...
		public void Reset()
		{
            ...
            m_AverageValue[1] = 0;
                        if (ResetEvent  != null)
			ResetEvent(this, new AudioLib.Events.VuMeter.ResetEventArgs());
					}
		...
		public void CatchUpdateVuMeterEvent ( object sender , AudioLib.Events.Player.UpdateVuMeterEventArgs Update) 
		{
			boolPlayer = true ;
		...
		public void CatchUpdateVuMeterEvent (object sender , AudioLib.Events.Recorder.UpdateVuMeterEventArgs UpdateVuMeter)
		{
        ...
        private void ComputePeakDbValue()
        {
            ...
            if ( UpdatePeakMeter != null )
                            UpdatePeakMeter(this, new AudioLib.Events.VuMeter.UpdatePeakMeter(maxDbs));
		...
		void AnimationComputation ()
		{
            ...
            */
            ...
            if ( UpdateForms != null )
            UpdateForms(this, new AudioLib.Events.VuMeter.UpdateFormsEventArgs());
		}
        ...
        private void DetectLowAmplitude()
        {
            ...
            if (LowAmpList.Count > 4)
                ...
                if (avg < m_UBound)
                    {
                    if (LevelTooLowEvent != null) LevelTooLowEvent ( this, new AudioLib.Events.VuMeter.LevelTooLowEventArgs ( this, avg, 0, 0 ) );
                    ...
                    {
                    m_IsLowAmplitude = false;
                    if (LevelGoodEvent != null) LevelGoodEvent (this ,  new AudioLib.Events.VuMeter.LevelGoodArgs ( avg ) );
                    }
                LowAmpList.Clear ();
                LowThresholdCount = 0;
