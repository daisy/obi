    ...
    public partial class WavePanel : UserControl
    {
        ...
        private void WavePanel_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (mWaveFile != null)
            {
                ...
                if (mWaveFile.Format.BitsPerSample == 8)
                {
                    Draw8Bits(e.Graphics, spp, pen1);
                }
                else
                {
        ...
        private void Draw8Bits(Graphics g, double spp, Pen pen1)
        {
            ...
            while (sample < mWaveFile.Samples)
            {
                ...
                {
                    byte v = mWaveFile.Bytes[s];
                    if (v < min) min = v;
                    if (v > max) max = v;
                    ++samplesRead;
                }
                int ymin = Height - (min * Height) / byte.MaxValue;
                int ymax = Height - (max * Height) / byte.MaxValue;
                ...
                if (mWaveFile.Format.Channels == 2)
                {
                    ...
                    ymin = Height - (min * Height) / byte.MaxValue;
                    ymax = Height - (max * Height) / byte.MaxValue;
