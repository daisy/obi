...
namespace Protobi
{
    ...
    public class WaveFile
    {
        ...
        private short[] mShorts;       // for 16-bit samples
        ...
        public WaveFile(string path)
        {
            ...
            while (bytesLeft > 0)
            {
                ...
                if (hd.id == "fmt ")
                {
                    if (mFormat != null)
                    {
                        throw new Exception(Localizer.GetString("unsupported_format"));
                    }
                ...
                else if (hd.id == "data")
                {
                    if (mFormat == null)
                    {
                        ...
                        throw new Exception(Localizer.GetString("unsupported_data"));
                    }
        ...
        private uint ReadHeader(BinaryReader reader)
        {
            ...
            if (hd.id != "RIFF")
            {
                throw new Exception(String.Format(Localizer.GetString("not_riff"), hd.id));
            }
            byte[] buf = reader.ReadBytes(4);
            if (Encoding.ASCII.GetString(buf) != "WAVE")
            {
                throw new Exception(String.Format(Localizer.GetString("unsupported_riff_type"), Encoding.ASCII.GetString(buf)));
            }
        ...
        private uint ReadData(BinaryReader reader, uint size)
        {
            ...
            {
                throw new Exception(String.Format(Localizer.GetString("unsupported_bits_per_sample"), mFormat.BitsPerSample));
            }
            return size;
        }
        ...
        public class WaveFormat
        {
            ...
            public static WaveFormat Read(BinaryReader reader)
            {
                ...
                if (tag != PCM)
                {
                    throw new Exception(String.Format(Localizer.GetString("unknown_format_tag"), tag));
                }
